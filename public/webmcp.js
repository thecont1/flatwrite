...
// module's exports change. index.html carries ?v=9 for this file —
...
if (mc === null) {
  ...
  var EXECUTORS = {
    ...
    // === assist_document ===
    assist_document: async function (args) {
      args = args || {};
      try {
        var bridge = requireBridge();
        if (typeof bridge.assistDocument !== 'function') {
          return errorResult('BRIDGE_UNAVAILABLE', 'Assist is not available on this page', true);
        }
        var md = typeof args.markdown === 'string' ? args.markdown : null;
        if (md == null) {
          var state = bridge.getDocumentState ? bridge.getDocumentState() : null;
          // Prefer live editor content via update path: app.js assist uses editor.value
          md = (typeof window !== 'undefined' && window.document && window.document.getElementById('editor'))
            ? (window.document.getElementById('editor').value || '')
            : '';
        }
        var selection = null;
        if (typeof args.selectionStart === 'number' && typeof args.selectionEnd === 'number') {
          selection = {
            start: args.selectionStart,
            end: args.selectionEnd,
            text: typeof args.selectionText === 'string'
              ? args.selectionText
              : md.slice(args.selectionStart, args.selectionEnd),
          };
        }
        var result = await bridge.assistDocument({
        ...
        return {
          content: [{
            type: 'text',
            text: (result.explanation || 'Assist complete') +
              (result.model ? ' [' + result.model + ']' : '') +
              '. Apply with update_document_content if desired.',
          }],
          structuredContent: Object.assign(
            { nextSuggestedTool: 'update_document_content' },
            result,
          ),
        };
      } catch (e) {
        return errorResult(e.code || 'ASSIST_FAILED', e.message || String(e), e.retryable);
      }
    },
